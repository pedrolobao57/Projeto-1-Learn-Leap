...
function generateRandomTeachers(num = 20000) {
  ...
  const locations = ["Aveiro", "Beja", "Braga", "Bragança", "Castelo Branco", "Coimbra", "Évora", "Faro", "Guarda", "Leiria", "Lisboa", "Portalegre", "Porto", "Santarém", "Setúbal", "Viana do Castelo", "Vila Real", "Viseu"];
  ...
  function randomName() {
const firstNames = [
  "Afonso", "Alexandre", "Alfredo", "Amélia", "Ana", "André", "António", "Aurélio", "Beatriz", "Benedita",
  "Bruno", "Catarina", "Cláudia", "Clara", "Cristina", "Daniel", "Diana", "Diogo", "Duarte", "Eduarda",
  "Eduardo", "Filipe", "Francisca", "Francisco", "Gabriel", "Gonçalo", "Helena", "Henrique", "Inês", "Isabel",
  "Joana", "Joaquim", "João", "José", "Júlia", "Leonor", "Lourenço", "Luana", "Lucas", "Luís",
  "Madalena", "Manuel", "Maria", "Mariana", "Marta", "Matilde", "Miguel", "Patrícia", "Rafael", "Rita"
];
    const lastNames = [
  "Teixeira", "Araújo", "Freitas", "Machado", "Castro", "Farias", "Vieira", "Nascimento", "Cardoso", "Monteiro",
  "Antunes", "Macedo", "Amaral", "Tavares", "Peixoto", "Pacheco", "Queiroz", "Rezende", "Aguiar", "Azevedo",
  "Fonseca", "Menezes", "Batista", "Campos", "Cavalcanti", "Figueiredo", "Garcia", "Borges", "Nogueira", "Abreu",
  "Lima", "Sales", "Leal", "Albuquerque", "Torres", "Barros", "Assis", "Moreira", "Rezende", "Valente",
  "Lacerda", "Cunha", "Melo", "Guimarães", "Sarmento", "Bittencourt", "Lourenço", "Meireles", "Brandão", "Marques"
];
...
function filterTeachersByStudentPrefs(teachers) {
  const studentPrefsArray = JSON.parse(localStorage.getItem("studentAccounts")) || [];
  const studentPrefs = studentPrefsArray.length > 0 ? studentPrefsArray[0] : {};

  const { subject, classes: classType, level, location, maxPrice, days } = studentPrefs;

  if (!subject || !classType || !level || !location || !maxPrice || !days || days.length === 0) {
    return [];
  }

  return teachers.filter((teacher) => {
    const matchesDays = teacher.days.some((day) => days.includes(day));

    return (
      teacher.subject === subject &&
      (classType === "Both" || teacher.classType === classType || teacher.classType === "Both") &&
      teacher.level === level &&
...
let allTeachers = loadTeachers();
let teachers = filterTeachersByStudentPrefs(allTeachers);
let currentIndex = teachers.length - 1;

function createCard(teacher) {
  const card = document.createElement("div");
  card.classList.add("cardContainer", "absolute", "w-full", "h-full", "bg-white", "rounded-xl", "border", "border-blue-300", "transition-transform", "duration-300", "ease-in-out");
  ...
  if (teachers.length === 0) {
    cardContainer.innerHTML = `
      <div class="text-center mt-20">
        <p class='text-gray-400 mb-4'>No teachers match your preferences.</p>
        <button id="changePrefsBtn" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition">Change Preferences</button>
      </div>
    `;
